...
sorted_networks=$(echo -e "$toggle\n$wifi_list" | sort | uniq )
selected_network=$(echo -e "$sorted_networks\n$nmtui" | rofi -dmenu -i -selected-row 1 -p "Wi-Fi SSID: " -theme ${config_dir}/wi-fi )
...
elif [ "$selected_network" = "$nmtui" ]; then
	ghostty -e nmtui
else
